...
- These permissions determine what actions can be performed within the Takaro API. You can refer to the [API guide](/advanced/api) for usage examples and the [API Reference](pathname:///api-docs/) for the complete type reference.
...
It depends on your game. For 7 Days to Die, Takaro connects to your server, so the relevant port must be accessible from the internet. For Rust, Minecraft, Hytale, and other games using the generic connector, your server connects outbound to Takaro, so no port forwarding or inbound firewall rules are needed.
